...
export function CreateCurriculum({ onNavigate }: Props) {
  ...
  const [description, setDescription] = useState("Competency-Based Curriculum for Junior Secondary\n(Grades 7-9) aligned with the latest CBC framework.");
  ...
  const [cycleModel, setCycleModel] = useState("3terms");
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">Build a structured learning journey that can be deployed to one or more schools.</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => onNavigate("management")} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 bg-white hover:bg-gray-50">
              Cancel
            </button>
          ...
          {steps.map((step, i) => (
            <div key={step} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center">
                ...
                <span className={`text-xs mt-1 whitespace-nowrap ${i === 0 ? "text-[#1a4db5] font-medium" : "text-gray-400"}`}>{step}</span>
              </div>
          ...
          <div className="col-span-3 space-y-6">
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Curriculum Code <span className="text-red-500">*</span></label>
                  <input
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Curriculum Framework <span className="text-red-500">*</span></label>
                    <div className="relative">
                      <select
                        value={framework}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Academic Cycle Model <span className="text-red-500">*</span></label>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      >
                        <opt.icon size={20} className={`mx-auto mb-1 ${cycleModel === opt.id ? "text-[#1a4db5]" : "text-gray-400"}`} />
                        <div className={`text-xs font-semibold ${cycleModel === opt.id ? "text-[#1a4db5]" : "text-gray-700"}`}>{opt.label}</div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-lg">
                    <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full flex items-center gap-1">
                      Kenya
                      <button className="ml-1 text-blue-500 hover:text-blue-700"><X size={10} /></button>
                    </span>
                  ...
                  <div className="flex items-center flex-wrap gap-2 px-3 py-2 border border-gray-300 rounded-lg">
                    {["CBC", "Junior Secondary", "Core Curriculum"].map(tag => (
                      <span key={tag} className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full flex items-center gap-1">
                        {tag}
                        <button className="text-gray-400 hover:text-gray-600"><X size={10} /></button>
                      </span>
                    ))}
              ...
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center gap-2">
                ...
                <p className="text-xs text-blue-700">You can always edit these details later. Once published, a new version will be created for major changes.</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                { icon: GitBranch, label: "Version Controlled", desc: "Every update creates a new version for easy tracking.", color: "text-blue-600", bg: "bg-blue-50" },
                { icon: CheckCircle2, label: "Standards-Aligned", desc: "Map competencies and outcomes to learning standards.", color: "text-green-600", bg: "bg-green-50" },
                { icon: Rocket, label: "Flexible Deployment", desc: "Deploy to one or multiple schools with custom settings.", color: "text-purple-600", bg: "bg-purple-50" },
                { icon: BarChart3, label: "Full Visibility", desc: "Track progress and performance across all schools.", color: "text-orange-600", bg: "bg-orange-50" },
              ].map((f) => (
                ...
                <div className="p-4 bg-gray-50 flex items-center gap-3 border-b border-gray-200">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      {cycleModel === "3terms" ? "3 Terms" : cycleModel === "2sem" ? "2 Semesters" : "Custom"} • Grades 7 - 9
                    </div>
                  </div>
                </div>
                <div className="p-4">
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    {(cycleModel === "3terms" ? ["Term 1", "Term 2", "Term 3"] : cycleModel === "2sem" ? ["Semester 1", "Semester 2"] : ["Period 1"]).map((t) => (
